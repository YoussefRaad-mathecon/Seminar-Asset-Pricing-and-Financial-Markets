...
class HestonModel:
    ...
    def generateHestonPathQEMDisc(self, T, n, C):
        ...
        for i in range(1, n + 1):
            ...
            if psi <= C:
                ...
                K0star = -((A * b2 * a) / (1 - 2 * A * a)) + ((1 / 2) * np.log(1 - 2 * A * a)) - (K1 + (1 / 2) * K3) * v[i-1]  +  self.r * dt
                v_next = a * (np.sqrt(b2) + Zv)**2
            else:  # psi > C
                p = (psi - 1) / (psi + 1)
                beta = (1 - p) / m
                ...
                K0star = -np.log(p + (beta * (1 - p)) / (beta - A)) - (K1 + (1 / 2) * K3) * v[i-1]  +  self.r * dt
                if (0 <= Uv <= p):
                    v_next = 0
                elif (p < Uv <= 1):
                    v_next = (1 / beta) * np.log((1 - p) / (1 - Uv))
            ...
            # Calculate log-price (with drift (risk-free rate under Q(lambda_)))
            Zs = Zs_array[i - 1]
            log_Si = np.log(S[i - 1]) + K0star + K1 * v[i - 1] + K2 * v[i] + np.sqrt(K3 * v[i - 1] + K4 * v[i]) * Zs
            S[i] = np.exp(log_Si)  
    ...
    def generateHestonPathBKDI(self, T, n):
        v_zero_count = 0
        d = 4 * self.theta_tilde * self.kappa_tilde / self.sigma**2
        exp_kappa_t = np.exp(-self.kappa_tilde * T)

        S = np.zeros(n + 1)  # Adjust size to include initial value
        S[0] = self.S0  # Set initial value

        for i in range(n):
            lambda_param = 4 * self.kappa_tilde * exp_kappa_t / (self.sigma**2 * (1 - exp_kappa_t)) * self.v0

            if d > 1:
                chi_d_minus_1 = chi2.rvs(d - 1)
                Z = norm.rvs()
                V_t = self.sigma**2 * (1 - exp_kappa_t) / (4 * self.kappa_tilde) * (Z + np.sqrt(lambda_param))**2 + chi_d_minus_1
            else:
                N_pois = poisson.rvs(0.5 * lambda_param)
                V_t = chi2.rvs(d + 2 * N_pois)

            integrated_variance = 0.5 * (self.v0 + V_t)
            sqrt_variance_integral = (1 / self.sigma) * (V_t - self.v0 - self.kappa_tilde * self.theta_tilde * T + self.kappa_tilde * integrated_variance)
            m = np.log(self.S0) + (self.r * T - 0.5 * integrated_variance + self.rho * sqrt_variance_integral)
            sigma = np.sqrt((1 - self.rho**2) * integrated_variance)
            Z_stock = norm.rvs()
            S[i + 1] = np.exp(m + sigma * Z_stock)  # Update price for next step
    ...
    def priceHestonCallViaMC(self, K, T, n, num_paths, method):
        ...
        for i in range(num_paths):
            if method == "TGMDisc":
                ...
                S, v_zero_count = self.generateHestonPathBKDI(T, n)
            elif method == "TGDisc":
        ...
        # Confidence Interval
        CI = (Zc * std_dev) / np.sqrt(num_paths)
        
        return price, v_zero_counts, CI
